  ...
  $.ajax({
    ...
    if(msg.success == "true") {
      ...
      msg.rows.forEach(function(row) {
          $('#hubs-table').find('tbody').append('<tr><th scope="row">' + row.id + '</th><td>' + row.create_date + '</td><td><a href="' + ECHIDNA_URL + '?page=hub&hub_id=' + row.id + '&offset=0">' + row.hub_name + '</a></td><td>' + row.roles_count + '</td><td>' + row.posts_count + '</td><td>' + row.user_role + '</td></tr>');
      });
